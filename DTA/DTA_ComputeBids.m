...
function [DTA_Data bestIdxs taskTimes feasibility] = DTA_ComputeBids(DTA_Params, DTA_Data, agent, tasks, feasibility)


% If the path is full then we cannot add any tasks to it
L = find(DTA_Data.path == -1);
if( isempty(L) )
    return;
end

% Reset bids, best positions in path, and best times
DTA_Data.bids = zeros(1, DTA_Params.M);
bestIdxs       = zeros(1, DTA_Params.M);
taskTimes      = zeros(1, DTA_Params.M);

% For each task
for m=1:DTA_Params.M
    
    % Check for compatibility between agent and task
    if(DTA_Params.CM(agent.type, tasks(m).type) > 0),
               
        % Check to make sure the path doesn't already contain task m
        if( isempty(find(DTA_Data.path(1,1:L(1,1)-1) == m)) )
            ...
            for j=1:L(1,1)
                if( feasibility(m,j) == 1 )
                    ...
                    if(j == 1) % insert at the beginning
                        ...
                        taskPrev = tasks(DTA_Data.path(j-1));
                        timePrev = DTA_Data.times(j-1);
                    end
                    ...
                    if(j == L(1,1)) %insert at the end
                        ...
                        taskNext = tasks(DTA_Data.path(j));
                        timeNext = DTA_Data.times(j);
                    end
                    
                    % Compute min and max start times and score
                    [score minStart maxStart] = Scoring_CalcScore(DTA_Params,agent,tasks(m),taskPrev,timePrev,taskNext,timeNext);
