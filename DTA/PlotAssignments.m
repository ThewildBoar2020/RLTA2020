...
function fig = PlotAssignments(WORLD, DTA_Assignments, agents, tasks, figureID)
...
for n=1:length(agents)
    ...
    if( DTA_Assignments(n).path(1) > -1 )
        taskPrev = lookupTask(tasks, DTA_Assignments(n).path(1));
        X = [agents(n).x, taskPrev.x];
        Y = [agents(n).y, taskPrev.y];
        T = [0, DTA_Assignments(n).times(1)];
        ...
        agents(n).time = round(DTA_Assignments(n).times(1)/100);
        agents(n).path = [taskPrev.x, taskPrev.y, agents(n).z, taskPrev.duration/100, taskPrev.id];
        
        for m = 2:length(DTA_Assignments(n).path);
            if( DTA_Assignments(n).path(m) > -1 )
                taskNext = lookupTask(tasks, DTA_Assignments(n).path(m));
                X = [taskPrev.x, taskNext.x];
                Y = [taskPrev.y, taskNext.y];
                T = [DTA_Assignments(n).times(m-1)+taskPrev.duration, DTA_Assignments(n).times(m)];
                plot3(X,Y,T,'-^','color',Cmap(agents(n).type,:));
...
for n=1:length(agents),
    ...
    grid on;
    axis([0 1600 0 2])
    for m = 1:length(DTA_Assignments(n).path),
        if (DTA_Assignments(n).path(m) > -1)
            taskCurr = lookupTask(tasks, DTA_Assignments(n).path(m));
            plot([DTA_Assignments(n).times(m) DTA_Assignments(n).times(m)+taskCurr.duration],[1 1],'-','color',Cmap(agents(n).type,:), 'Linewidth',10)
...
for n=1:length(agents),
    UAV = agents(n);
    File = strcat(('\agent'),int2str(UAV.id));
    save(File, 'UAV');
end
