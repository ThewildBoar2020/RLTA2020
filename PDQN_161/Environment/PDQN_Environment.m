function [SurObnext,Reward,x,goal,state]=PDQN_Environment(evalParam,Kinematic,x,goal,Ob,Const)
% 	Reward=0;
	PreDist=norm(x(1:2)-goal);
	[xnext,~]=PDQN_LocalPathPlanning(Ob,goal,x,Kinematic,evalParam,10);
...
%         disp(['Arrive Goal ',num2str(T),' !!']);
        ...
        goal=PDQN_GoalRandGen(xnext(1:2),Ob);%GUO
        %T=T+1;
        state=1;
    end
    [SurObnext,~]=PDQN_GetCurSurOb(xnext,goal,Ob);
	x=xnext;
end